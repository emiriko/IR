...
class InvertedIndexReader(InvertedIndex):
    ...
    def __next__(self):
        ...
        start_position, number_of_postings, length_in_bytes_of_postings = self.postings_dict[term]

        self.index_file.seek(start_position)

        posting_list_encoded = self.index_file.read(length_in_bytes_of_postings)
        posting_list_decoded = self.encoding_method.decode(posting_list_encoded)

        return term, posting_list_decoded
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list):
        """
        Menambahkan (append) seb`uah term dan juga postings_list yang terasosiasi
        ...
        self.postings_encoding = self.encoding_method.encode(postings_list)

        start_position = self.index_file.tell()
        # Menyimpan metadata dalam bentuk self.terms
        self.terms.append(term)

        # Menyimpan metadata self.postings_dict
        number_of_postings = len(postings_list)
        length_in_byte = len(self.postings_encoding)
        self.postings_dict[term] = (start_position, number_of_postings, length_in_byte)

        # Menambahkan (append) bystream dari postings_list yang sudah di-encode ke posisi akhir index file di harddisk.

        self.index_file.write(self.postings_encoding)
...
if __name__ == "__main__":
    ...
    with InvertedIndexWriter('test', encoding_method=VBEPostings, path='./tmp/vb') as index:
        index.append(1, [2, 3, 4, 8, 10])
        index.append(2, [3, 4, 5])
        index.index_file.seek(0)
        assert index.terms == [1,2], "terms salah"
    ...
    with InvertedIndexWriter('test', encoding_method=EliasGammaPostings, path='./tmp/eg') as index:
        index.append(1, [2, 3, 4, 8, 10])
        index.append(2, [3, 4, 5])
        index.index_file.seek(0)
        assert index.terms == [1, 2], "terms salah"
        ...
        assert EliasGammaPostings.decode(index.index_file.read()) == [2, 3, 4, 8, 10], "penyimpanan postings pada harddisk salah"

        index.index_file.seek(index.postings_dict[2][0])
        assert EliasGammaPostings.decode(index.index_file.read(len(EliasGammaPostings.encode([3, 4, 5])))) == [3, 4,
                                                                                                 5], "terdapat kesalahan"

        index.index_file.seek(0)
        assert EliasGammaPostings.decode(index.index_file.read(index.postings_dict[1][2])) == [2, 3, 4, 8,
                                                                                        10], "terdapat kesalahan"
