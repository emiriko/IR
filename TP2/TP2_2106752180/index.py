...
class InvertedIndexReader(InvertedIndex):
    ...
    def __next__(self):
        ...
        posting_list, tf_list = self.get_postings_list(term)

        return term, posting_list, tf_list
    ...
    def get_postings_list(self, term):
        ...
        posting_list_encoded = self.index_file.read(length_in_bytes_of_postings_list)
        posting_list = self.postings_encoding.decode(posting_list_encoded)

        tf_list_encoded = self.index_file.read(length_in_bytes_of_tf_list)
        tf_list = self.postings_encoding.decode_tf(tf_list_encoded)

        return posting_list, tf_list
    ...
    def get_average_document_length(self):
        ...
        number_of_documents = len(self.doc_length)
        total_document_Lengths  = 0

        for value in self.doc_length.values():
            total_document_Lengths += value

        return total_document_Lengths/number_of_documents
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        start_position, len(postings_list), len(encoded_postings_list), len(encoded_tf_list))

        for term, frequency in zip(postings_list, tf_list):
            if term in self.doc_length:
                self.doc_length[term] += frequency
            else:
                self.doc_length[term] = frequency
